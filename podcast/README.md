...
selectable voices for an episode, use `generate_all_voices.sh` (below).

Output is `.m4a` (AAC), small and plays everywhere.
...
```bash
KOKORO_VOICE=bm_george KOKORO_VOICE2=bf_emma ./generate_audio.sh   # British pair
KOKORO_SPEED=1.1 ./generate_audio.sh                               # a touch faster
```
...
## TTS engine fallbacks

`generate_audio.sh` auto-picks an engine: **Kokoro** (neural, preferred) →
**Piper** (if installed, `PIPER_MODEL` set) → macOS **`say`** (built-in).
Force one with `ENGINE=say` etc. Note: the two-voice format and `[pause]` are
only honoured by the Kokoro engine.
